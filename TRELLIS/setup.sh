...
case $PLATFORM in
    ...
    hip)
        ...
        echo "[SYSTEM] Installing PyTorch 2.4.1 for HIP ($PYTORCH_VERSION -> 2.4.1+rocm6.1)"
            pip3 install torch==2.4.1 torchvision==0.19.1 --index-url https://download.pytorch.org/whl/rocm6.1 --user
...
#                 2.4.0+cu121) pip3 install xformers==0.0.27.post2 --index-url https://download.pytorch.org/whl/cu121 ;;
...
# Install warp
mkdir -p /tmp/extensions
git clone --recursive https://github.com/NVIDIA/warp /tmp/extensions/warp
...
pip3 install /tmp/extensions/nvdiffrast
...
pip3 install /tmp/extensions/diffoctreerast
...
pip3 install /tmp/extensions/mip-splatting/submodules/diff-gaussian-rasterization/
...
if [ "$VOX2SEQ" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        ...
        pip3 install /tmp/extensions/vox2seq
    else
...
#             11) pip3 install spconv-cu118 ;;
#             12) pip3 install spconv-cu120 ;;
...
if [ "$DEMO" = true ] ; then
    pip3 install gradio==4.44.1 gradio_litmodel3d==0.0.1
fi
