...
if [ $venv_flag == 0 ]; then
        ...
        if [ -z "$( ls -A 'container' )" ]; then
                apptainer pull container/test_x2sim_container_0.1.sif docker://lsmith9003/test_x2sim_container:0.1
        else
                echo "Using existing sif file found in ./container"
        fi
        echo "Running x2sim through docker container"
        apptainer exec --nv --env OPEN_API_KEY=$OPEN_API_KEY container/test_x2sim_container_0.1.sif python3 x2sim/x2sim.py "$@"
